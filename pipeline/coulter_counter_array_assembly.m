...
%%
% get data from csv file from compiled sheet

[instruction_filename, instruction_dir] = uigetfile('../*.*','Select File',' ');

addpath(instruction_dir)

[~,sheet_name]=xlsfinfo(instruction_filename);
data = xlsread(instruction_filename,sheet_name{end});

%%
% create separate csv file for each sample

instruction_dir = uigetdir('../*.*','Where to save the fitting data?');

size_data = size(data);

samples = {'all_beads','beads_6um','beads_10um','beads_15um','beads_25um','8902_rep1','8902_rep2','8902_rep3','baf3_rep1','baf3_rep2','baf3_rep3',...
    'dt40_rep1','dt40_rep2','dt40_rep3','l1210_rep1','l1210_rep2','l1210_rep3','sHela_rep1','sHela_rep2','sHela_rep3','thp1_rep1','thp1_rep2','thp1_rep3'};

k=1;
for i = 1:2:size_data(2)
    volumes = data(:,i);
    counts = data (:,i+1);
    extended_vol = repelem(volumes,counts);
    current_sample = table(extended_vol);
    cd(instruction_dir)
    filename = [samples{k},'_volumes.csv'];
    writetable(current_sample, filename,'WriteVariableNames',0);
    k = k+1;
end
cd(currentFolder)
...
paired_data_var_names = {'volume_fL'};
...
FBM_var_to_include = {'volume_fL'};
%FBM_var_to_include = {'buoyant_mass','pmt1','pmt2','pmt3'}; % for older readout_pairing version
instruct_var_to_exclude = {'path','filename','pmt_1_label','pmt_2_label','pmt_3_label','pmt_4_label','pmt_5_label'}; % metadata sheet will include all variables from the instruction sheet except variables specified here
